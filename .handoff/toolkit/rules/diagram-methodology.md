...
- `code_refs` is omitted for these typed/business documents (ADR, runbook, onboarding guide, API summary, config reference, glossary) — they carry no inline code snippets, so there is nothing to navigate to. (This is unlike `handover_node` domain nodes, which DO emit `code_refs` — one per inline snippet — so the extension can navigate to the live source. See `output-schema.md` FM-09.)
